...
// ── network (sysctl NET_RT_IFLIST2 → if_data64 cumulative bytes → per-second rate) ──
// ⚠️ 别用 getifaddrs：macOS 下它的 `ifa_data` 指向 `struct if_data`（`ifi_baudrate` 是 **u32**），
// 当成 `if_data64`（`ifi_baudrate` **u64**）读会从 baudrate 起整体错位 4 字节 → ibytes/obytes
// 读成垃圾（实测 rx 恒 0、tx 少算：报 48GB 而 netstat 单 en0 已 157GB）。NET_RT_IFLIST2 的
// `if_msghdr2` 内嵌的才是**真 if_data64**（64 位计数），本 IfData64 结构体对它正确。
// 跳 loopback（ifi_type==IFT_LOOP）；全接口 rx/tx 累加，两次差 / 时间差 = bytes/sec（首调 rate=0）。

const CTL_NET: c_int = 4;
const PF_ROUTE: c_int = 17;
const NET_RT_IFLIST2: c_int = 6;
const RTM_IFINFO2: u8 = 0x12;
const IFT_LOOP: u8 = 0x18; // 24 = loopback
...
// if_msghdr2（<net/if.h>）—— NET_RT_IFLIST2 每条接口记录头，尾部内嵌 if_data64。
// extern struct 按 C ABI 布局（u16 ifm_index 后自动 pad 到 int 对齐），别手算偏移。
const IfMsgHdr2 = extern struct {
    ifm_msglen: u16 = 0,
    ifm_version: u8 = 0,
    ifm_type: u8 = 0,
    ifm_addrs: i32 = 0,
    ...
    ifm_index: u16 = 0,
    ifm_snd_len: i32 = 0,
    ifm_snd_maxlen: i32 = 0,
    ifm_snd_drops: i32 = 0,
    ifm_timer: i32 = 0,
    ifm_data: IfData64 = .{},
};

extern "c" fn sysctl(name: [*]const c_int, namelen: c_uint, oldp: ?*anyopaque, oldlenp: *usize, newp: ?*const anyopaque, newlen: usize) c_int;
...
// session 累计（自插件启动）——由每拍 wrapping 差累加，免 32 位计数器环回。rx_total/tx_total 语义
// = 「本次会话累计」（非自开机；内核 if_data64 字节计数只低 32 位可靠，自开机 64 位总量拿不到）。
var session_rx_total: u64 = 0;
var session_tx_total: u64 = 0;
...
fn sampleNetTotals() !NetTotals {
    var mib = [_]c_int{ CTL_NET, PF_ROUTE, 0, 0, NET_RT_IFLIST2, 0 };
    var len: usize = 0;
    if (sysctl(&mib, mib.len, null, &len, null, 0) != 0) return error.OSCall;
    // 接口表通常几 KB；固定栈缓冲，异常大（不该发生）→ 报错回落 rate=0。
    var buf: [64 * 1024]u8 = undefined;
    if (len == 0 or len > buf.len) return error.OSCall;
    if (sysctl(&mib, mib.len, &buf, &len, null, 0) != 0) return error.OSCall;
    ...
    var off: usize = 0;
    while (off + 4 <= len) {
        const msglen: usize = std.mem.readInt(u16, buf[off..][0..2], native); // ifm_msglen
        if (msglen == 0 or off + msglen > len) break;
        const mtype = buf[off + 3]; // ifm_type
        // RTM_IFINFO2 记录才含 if_data64；memcpy 进对齐结构体（sysctl 记录未必 8 字节对齐，
        // 不能 pointer-cast），越界守卫 off+sizeof<=len。其它记录（RTM_NEWADDR2 等）跳过。
        if (mtype == RTM_IFINFO2 and off + @sizeOf(IfMsgHdr2) <= len) {
            var hdr: IfMsgHdr2 = .{};
            @memcpy(std.mem.asBytes(&hdr)[0..@sizeOf(IfMsgHdr2)], buf[off..][0..@sizeOf(IfMsgHdr2)]);
            if (hdr.ifm_data.ifi_type != IFT_LOOP) {
                // ⚠️ macOS 实测：if_data64 的 ifi_ibytes/ifi_obytes 对部分 NIC（如 en0）只填**低
                // 32 位**（高 32 位恒 0；netstat -ib 的 64 位值另有来源，逐字段 hexdump 证实偏移
                // 正确、纯是内核只填低 32 位）。故当**累计计数器的低 32 位环回**处理：取 u32，
                // netSample 用 wrapping u32 差累加成 u64 session 总量。这里只累加原始低 32 位。
                rx +%= @as(u32, @truncate(hdr.ifm_data.ifi_ibytes));
                tx +%= @as(u32, @truncate(hdr.ifm_data.ifi_obytes));
            }
        }
        off += msglen;
    }
    // rx/tx = 全接口低 32 位之和（可能已跨过多个 4GB，但对「相邻两拍差」仍正确 —— 见 netSample）。
    return .{ .rx = rx, .tx = tx, .ts_ms = netNowMs() };
}

fn netSample() NetSample {
    const now = sampleNetTotals() catch {
        return .{ .rx_bytes_per_sec = 0, .tx_bytes_per_sec = 0, .rx_total = session_rx_total, .tx_total = session_tx_total };
    };
    defer prior_net = now;
    const prev = prior_net orelse {
        // 首拍：无基线，rate=0，session 总量从 0 起累计。
    ...
    if (dt_ms > 0) {
        ...
        // now/prev 是全接口低 32 位之和，正常单调增。某接口 4GB 环回或接口增删 → 差为负 →
        // 该拍跳过（rate=0、总量不加），下一拍恢复。正差累加进 u64 session 总量（rx/tx 独立判，
        // 一方环回不牵连另一方）。
        if (now.rx >= prev.rx) {
            const d_rx = now.rx - prev.rx;
            session_rx_total += d_rx;
            rx_rate = @as(f64, @floatFromInt(d_rx)) / dt_s;
        }
        if (now.tx >= prev.tx) {
            const d_tx = now.tx - prev.tx;
            session_tx_total += d_tx;
            tx_rate = @as(f64, @floatFromInt(d_tx)) / dt_s;
        }
    }
    return .{ .rx_bytes_per_sec = rx_rate, .tx_bytes_per_sec = tx_rate, .rx_total = session_rx_total, .tx_total = session_tx_total };
}
